...
def login_window():
    ...
    tk.Label(login_win, text="Username:", font=LABEL_FONT, bg=BG_COLOR).pack(pady=5)
    username_entry = tk.Entry(login_win, font=LABEL_FONT)
    username_entry.pack(pady=5)

    tk.Label(login_win, text="Password:", font=LABEL_FONT, bg=BG_COLOR).pack(pady=5)
    password_entry = tk.Entry(login_win, font=LABEL_FONT, show="*")
    ...
    def validate_login():
        ...
        if username == USERNAME and password == PASSWORD:
            messagebox.showinfo("Login Successful", "Welcome to the Student Mark System!")
            login_win.destroy()  # Close the login window
            root.deiconify()  # Show the main application window
        else:
            messagebox.showerror("Login Failed", "Invalid username or password")

    tk.Button(login_win, text="Login", command=validate_login, bg=BUTTON_COLORS[0], fg="white", font=BUTTON_FONT).pack(pady=10)
...
# Show the login window before the main application
login_window()
